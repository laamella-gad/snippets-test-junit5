...
import com.laamella.snippets_test_junit5.core.*;
...
public class AntlrGrammarTestFactory<L extends Lexer, P extends Parser> extends SnippetTestFactory<GrammarTestCase<L, P>> {
    ...
    private static <L extends Lexer, P extends Parser> List<String> preprocess(
        ...
        GrammarTestCase<L, P> lpGrammarTestCase = new GrammarTestCase<>(lexer, parser, tokenList, tree, errors);
        return Arrays.stream(actualGenerators).map(p -> p.generate(lpGrammarTestCase)).collect(Collectors.toList());
    }
